...
Two decoupled APIs plus a local demo frontend:

- Go/Fiber API for QR factorization and orchestration
- Node/Express API for matrix statistics
- Vite frontend for local walkthroughs and live testing
...
Docker was required by the challenge and makes all services reproducible locally and in future deployment environments.

## How the Rotation vs QR Ambiguity Was Handled

QR factorization was prioritized because it is the explicit and technically concrete requirement. The ambiguity is documented instead of broadening the scope inside the main contract.
...
- JWT auth is implemented on both services and is enabled by default, with `ENABLE_AUTH=false` available if a simpler local flow is needed
- A Go endpoint issues a demo JWT so auth can be tested end-to-end without inventing a user system
